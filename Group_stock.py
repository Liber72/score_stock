...
def Group_stock(group=''):
    ...
    if not all_data.empty:
        ...
        group1.to_csv(f'{group}_Group1.csv', index=False)
        group2.to_csv(f'{group}_Group2.csv', index=False)
        group3.to_csv(f'{group}_Group3.csv', index=False)

        # Xuất file tổng hợp (tùy chọn)
        output_data.to_csv(f'Top_{group}_All.csv', index=False)
    # Xuất CSV
