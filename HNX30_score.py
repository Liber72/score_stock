...
def Group_stock():
    ...
    for sym in group_stocks:
        ...
        if not history_data.empty:
            selected_data = history_data[['close', 'volume']].copy()
    ...
    if not all_data.empty:
        ...
        output_data = all_data[['symbol', 'close', 'volume', 'issue_share', 'marketcap', 
                               'rank_volume', 'rank_marketcap', 'total_score']]

        
        group1 = output_data.iloc[:10]  
        group2 = output_data.iloc[10:20]  
        group3 = output_data.iloc[30:]  

        group1.to_csv(f'HNX30_Group1.csv', index=False)
        group2.to_csv(f'HNX30_Group2.csv', index=False)
        group3.to_csv(f'HNX30_Group3.csv', index=False)

        # Xuất file tổng hợp (tùy chọn)
        output_data.to_csv(f'Top_HNX30_All.csv', index=False)
    # Xuất CSV
    all_data = all_data[['symbol', 'close', 'volume', 'issue_share', 'marketcap', 
                         'rank_volume', 'rank_marketcap', 'total_score']]
    all_data.to_csv(f'Top_HNX30.csv', index=False)
